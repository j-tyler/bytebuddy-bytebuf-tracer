...
public class AllocationRootTrackingTest {
    ...
    private void testHeapBufferAllocation() {
        ...
        tracker.recordMethodCall(heapBuffer, "Unpooled", "buffer", "Unpooled.buffer", heapBuffer.refCnt());
        ...
        // Release
        heapBuffer.release();
        tracker.recordMethodCall(heapBuffer, "HeapProcessor", "cleanup", "HeapProcessor.cleanup", heapBuffer.refCnt());
    ...
    private void testDirectBufferAllocation() {
        ...
        tracker.recordMethodCall(directBuffer, "Unpooled", "directBuffer", "Unpooled.directBuffer", directBuffer.refCnt());
        ...
        tracker.recordMethodCall(directBuffer, "DirectProcessor", "cleanup", "DirectProcessor.cleanup", directBuffer.refCnt());
    ...
    private void testWrappedBufferAllocation() {
        ...
        tracker.recordMethodCall(wrappedBuffer, "Unpooled", "wrappedBuffer", "Unpooled.wrappedBuffer", wrappedBuffer.refCnt());
        ...
        tracker.recordMethodCall(wrappedBuffer, "WrapperProcessor", "cleanup", "WrapperProcessor.cleanup", wrappedBuffer.refCnt());
    ...
    private void testCompositeBufferAllocation() {
        ...
        tracker.recordMethodCall(compositeBuffer, "Unpooled", "compositeBuffer", "Unpooled.compositeBuffer", compositeBuffer.refCnt());
        ...
        tracker.recordMethodCall(compositeBuffer, "CompositeProcessor", "cleanup", "CompositeProcessor.cleanup", compositeBuffer.refCnt());
    ...
    private void testPooledAllocator() {
        ...
        tracker.recordMethodCall(pooledBuffer, "PooledByteBufAllocator", "buffer", "PooledByteBufAllocator.buffer", pooledBuffer.refCnt());
        ...
        tracker.recordMethodCall(pooledBuffer, "PooledProcessor", "cleanup", "PooledProcessor.cleanup", pooledBuffer.refCnt());
    ...
    private void testUnpooledAllocator() {
        ...
        tracker.recordMethodCall(unpooledBuffer, "UnpooledByteBufAllocator", "buffer", "UnpooledByteBufAllocator.buffer", unpooledBuffer.refCnt());
        ...
        tracker.recordMethodCall(unpooledBuffer, "UnpooledProcessor", "cleanup", "UnpooledProcessor.cleanup", unpooledBuffer.refCnt());
    ...
    private void processBuffer(ByteBuf buffer, String processorName) {
        ...
        tracker.recordMethodCall(buffer, processorName, "validate", processorName + ".validate", buffer.refCnt());
        tracker.recordMethodCall(buffer, processorName, "process", processorName + ".process", buffer.refCnt());
    }
    ...
    public void testAllocationAsRootImprovedLeakDetection() {
        ...
        tracker.recordMethodCall(leakyDirectBuffer, "Unpooled", "directBuffer", "Unpooled.directBuffer", leakyDirectBuffer.refCnt());
        tracker.recordMethodCall(leakyDirectBuffer, "NetworkHandler", "handleRequest", "NetworkHandler.handleRequest", leakyDirectBuffer.refCnt());
        tracker.recordMethodCall(leakyDirectBuffer, "NetworkHandler", "processData", "NetworkHandler.processData", leakyDirectBuffer.refCnt());
        // Oops! Forgot to release - this is a leak

        // Create a properly released heap buffer
        ByteBuf goodHeapBuffer = Unpooled.buffer(512);
        tracker.recordMethodCall(goodHeapBuffer, "Unpooled", "buffer", "Unpooled.buffer", goodHeapBuffer.refCnt());
        tracker.recordMethodCall(goodHeapBuffer, "DataProcessor", "handle", "DataProcessor.handle", goodHeapBuffer.refCnt());
    ...
    public void testDeterministicRoots() {
        ...
        for (int i = 0; i < 5; i++) {
            ...
            tracker.recordMethodCall(buffer, "Unpooled", "buffer", "Unpooled.buffer", buffer.refCnt());

            // Different application paths
            if (i % 2 == 0) {
                tracker.recordMethodCall(buffer, "PathA", "process", "PathA.process", buffer.refCnt());
            } else {
                tracker.recordMethodCall(buffer, "PathB", "process", "PathB.process", buffer.refCnt());
            }

            buffer.release();
            String cleanupClass = i % 2 == 0 ? "PathA" : "PathB";
            tracker.recordMethodCall(buffer, cleanupClass, "cleanup", cleanupClass + ".cleanup", buffer.refCnt());
        }
