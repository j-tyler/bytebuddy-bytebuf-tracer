...
public class DirectOnlyFilteringTest {
    ...
    public void testAmbiguousBufferMethod_RequiresRuntimeCheck() {
        ...
        if (actuallyDirect) {
            tracker.recordMethodCall(buf, "UnpooledByteBufAllocator", "buffer", "UnpooledByteBufAllocator.buffer", buf.refCnt());
        }
    ...
    public void testWrappedBufferFilteringWithTrackDirectOnly() {
        ...
        if (wrappedHeapBuf instanceof io.netty.buffer.ByteBuf) {
            if (!wrappedHeapBuf.isDirect()) {
                ...
                tracker.recordMethodCall(wrappedHeapBuf, "Unpooled", "wrappedBuffer", "Unpooled.wrappedBuffer", wrappedHeapBuf.refCnt());
            }
        }
    ...
    public void testWrappedDirectBufferIsTracked() {
        ...
        if (wrappedDirectBuf.isDirect()) {
            tracker.recordMethodCall(wrappedDirectBuf, "Unpooled", "wrappedBuffer", "Unpooled.wrappedBuffer", wrappedDirectBuf.refCnt());
        }
    ...
    public void testMultiLevelWrapping_InheritsDirect() {
        ...
        tracker.recordMethodCall(wrapped2, "Unpooled", "wrappedBuffer", "Unpooled.wrappedBuffer", wrapped2.refCnt());
    ...
    public void testCompositeBuffer_MixedComponents() {
        ...
        if (isDirect) {
            ...
            tracker.recordMethodCall(composite, "Unpooled", "wrappedBuffer", "Unpooled.wrappedBuffer", composite.refCnt());
    ...
    public void testCompositeBuffer_AllHeapComponents() {
        ...
        if (composite.isDirect()) {
            tracker.recordMethodCall(composite, "Unpooled", "compositeBuffer", "Unpooled.compositeBuffer", composite.refCnt());
        }
